...
*Step-by-step build walkthrough with paste-ready VBA for the four Week-2 Access forms, built on the tested 13-table schema. Field names are assumed — map to `02-SIS-MVP-BUILD-SPEC-UPDATED.md`.*
...
**Prefer a one-click builder instead?** A `mod_CreateForms.bas` can build these forms programmatically (like `CreateAllTables`) — but it's harder to verify blind, so the designer + paste-VBA route above is the safer first pass.
