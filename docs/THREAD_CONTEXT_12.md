...
## 2026-07-27 (21:00) — Pilot site LOCKED + test files + outreach

**OpenCode session (final):**
1) **Pilot site LOCKED = 2 schools:** Allal (private vocational) + 1 public collège in El Bayadh. Private + public representation, manageable recruitment. Manuscript sections updated: abstract, §1.6, §3.3, §5.4, §6 conclusion — both EN and FR.
2) **6 test files ready for HyperAgent review** — grammar (A/B/C) + vocabulary (A/B/C) + TAALLIM_TEST_SPECS.md (7 files total). Ready to drag in.
3) **TAALLIM_TEACHER_INSTRUMENTS_RQ3.md saved** — bilingual Likert questionnaire (15 items + 2 open) + semi-structured interview guide (9 questions). Already applied to §3.5 in both manuscripts.
4) **TAALLIM_SCHOOL_OUTREACH.md created** — FR/AR email/WhatsApp for school directors + English teachers. Allal + public collège versions.
5) **TAALLIM_6WEEK_PILOT_PLAN.md updated** — section 0 now says "2 schools (locked)" instead of "4 middle schools"
6) **README.md updated** — reflects all new decisions (2 schools, test files, outreach, grade model)
7) **THREAD_CONTEXT_12.md updated** — full session log appended

**What's ready to send to HyperAgent:**
- Upload 7 test files (6 forms + TAALLIM_TEST_SPECS.md)
- Tell her: "Pilot site = Allal + 1 public collège (2 schools total)"
- She reviews grammar structure coverage, difficulty equivalence A/B/C, distractor quality, cultural appropriateness

**What's ready for school outreach:**
- Allal director (FR email, FR WhatsApp, AR email, AR WhatsApp)
- Allal English teacher (FR email, FR WhatsApp, AR WhatsApp)
- Public collège director (FR email, FR WhatsApp, AR email, AR WhatsApp)
...
2026-07-27 (20:30) — OpenCode session (tests + walkthrough):
1) SIS_ACCESS_WALKTHROUGH.md FIXED — now includes Step 1: create blank .accdb from scratch (no .accdb file existed before, only .bas modules)
2) TAALLIM_TEST_SPECS.md created — complete test specifications for both grammar and vocabulary (60 items each, 3 parallel forms A/B/C, counterbalancing strategy, validation protocol, scoring rules)
3) 6 ACTUAL TEST FILES CREATED:
   - GRAMMAR_TEST_FORM_A.md (pretest, 60 items, 55 scored + 5 buffer)
   - GRAMMAR_TEST_FORM_B.md (posttest, same structures, different lexical contexts)
   - GRAMMAR_TEST_FORM_C.md (delayed, same structures, different contexts again)
   - VOCABULARY_TEST_FORM_A.md (pretest, 60 items, 60 scored)
   - VOCABULARY_TEST_FORM_B.md (posttest, same items, different contexts)
   - VOCABULARY_TEST_FORM_C.md (delayed, same items, different contexts)
4) All tests include: complete answer keys, structure/word tracked per item, scoring rubrics, picture placeholders for vocabulary section
5) Grammar tests: 4 sections (MCQ, fill-blank, error correction, sentence building)
6) Vocabulary tests: 5 sections (picture matching, MCQ, fill-blank, translation AR→EN, sentence use)
7) Tests target middle school only (1AM–4AM), matching the pilot scope
8) Next: print forms, have 2 teachers review for equivalence, pilot on 30 non-participants to check item difficulty + discrimination before real pilot
...
## Deliverables This Session (v12)

| File | Purpose | Status |
|------|---------|--------|
| `SIS_ACCESS_WALKTHROUGH.md` | Step-by-step Access guide (create DB + import modules + run tables + seed data) | ✅ Fixed (was missing Step 1: create blank .accdb) |
| `TAALLIM_TEST_SPECS.md` | Complete test specifications (grammar + vocabulary, parallel forms, validation) | ✅ Created |
| `GRAMMAR_TEST_FORM_A.md` | Grammar pretest (60 items, 55 scored + 5 buffer) | ✅ Created |
| `GRAMMAR_TEST_FORM_B.md` | Grammar posttest (same structures, different contexts) | ✅ Created |
| `GRAMMAR_TEST_FORM_C.md` | Grammar delayed test (same structures, different contexts) | ✅ Created |
| `VOCABULARY_TEST_FORM_A.md` | Vocabulary pretest (60 items, 60 scored) | ✅ Created |
| `VOCABULARY_TEST_FORM_B.md` | Vocabulary posttest (same items, different contexts) | ✅ Created |
| `VOCABULARY_TEST_FORM_C.md` | Vocabulary delayed test (same items, different contexts) | ✅ Created |

## HyperAgent Next Tasks

1. **Review test items** — Check grammar forms for structure coverage (60 structures from 1AM–4AM), difficulty equivalence across forms, and distractor quality
2. **Review vocabulary items** — Check word selection (60 items from 640), cultural appropriateness, picture descriptions
3. **Draft teacher questionnaire** — Likert-scale adoption/satisfaction instrument for RQ3 (4 teachers)
4. **Draft teacher interview guide** — Semi-structured questions for post-pilot thematic analysis
5. **Update §3.6** — Table should reference the6 actual test forms (3 grammar + 3 vocabulary)
6. **Update taallim_analysis.py** — Ensure it handles both grammar and vocabulary datasets
7. **Validate form equivalence** — After teacher review, pilot on 30 non-participants; compute item difficulty + discrimination
8. **Lock pilot site** — Allal vs 4 El Bayadh schools (critical path)
